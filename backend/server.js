...
db.pragma('journal_mode = WAL');    // melhora concorrência leitura/escrita
db.pragma('synchronous = NORMAL');  // equilíbrio entre segurança e performance
...
app.post('/api/docs', (req, res) => {
  try {
    ...
    if (!title || !type || !html) {
      return res.status(400).json({ error: 'Campos obrigatórios: title, type, html' });
    }

    if (html.length > 500_000) { // ~500 KB
      return res.status(413).json({ error: 'Documento muito grande' });
    }
...
app.put('/api/docs/:id', (req, res) => {
  try {
    ...
    if (!title || !type || !html) {
      return res.status(400).json({ error: 'title, type e html não podem ficar vazios' });
    }
